...
* `goods` — товары (ID, название, цена);
* `sales` — продажи (ID продажи, ID товара, время продажи, количество);
* `good_sum_mart` — витрина данных (название товара, сумма продаж).

Необходимо реализовать триггер на таблице `sales`, который будет автоматически обновлять данные в витрине `good_sum_mart` при операциях `INSERT`, `UPDATE` и `DELETE`.

## Реализованное решение

### 1. Создание функции-триггера

Создана функция `sales_trigger()` на языке PL/pgSQL, которая обрабатывает все три типа операций:
...
* **Для `DELETE`:** вычитает стоимость удалённой продажи из суммы в витрине.
* **Для `INSERT`:**
    * если товар отсутствует в витрине — добавляет новую запись;
    * если товар уже есть — увеличивает сумму продаж.
* **Для `UPDATE`:** корректирует сумму продаж с учётом изменения количества товара.

### 2. Создание триггера

Триггер `sales_trg` привязан к таблице `sales` и вызывает функцию `sales_trigger()` после операций `INSERT`, `UPDATE`, `DELETE` для каждой строки:
...
Проведено тестирование всех сценариев работы триггера:
...
**Действия:**
```sql
INSERT INTO goods (goods_id, good_name, good_price) VALUES (3, 'Молоко', 100);
INSERT INTO sales (good_id, sales_qty) VALUES (3, 3);
```
...
Триггер корректно выполняет следующие задачи:
* поддерживает витрину данных в актуальном состоянии;
* обрабатывает все типы операций с таблицей `sales`;
* корректно работает с новыми товарами, добавляя их в витрину;
* учитывает изменения цен через связь с таблицей `goods`.
